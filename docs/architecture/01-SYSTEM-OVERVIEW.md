...
|----------|--------|---------------|
| API Gateway | ~~Не нужен~~ → Rust/Axum :8080, JWT verification, Redis sliding window rate limiting, reverse proxy routing to Python services, CORS (env-based origins), structured JSON request logging (X-Request-Id), health checks | Sprint 23 (in progress) |

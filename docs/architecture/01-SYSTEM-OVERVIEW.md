...
**Метрики:** 89 endpoint, 29 таблиц в 6 БД, 625 тестов, 157 RPS (p99 51ms).
...
| Enrollment | 8 | 4 | 39 | POST /enrollments, GET /me, lesson progress, auto-completion, course enrollment count, recommendations (co-enrollment collaborative filtering) |
| Payment | 12 | 8 | 128 | POST /payments (Stripe mock), subscription_plans, user_subscriptions, teacher_earnings, payouts, coupons, invoice PDF, refunds (admin approval workflow), GET /me |
| Notification | 6 | 2 | 38 | POST (email via Resend API or stub), GET /me, PATCH /:id/read, POST /streak-reminders/send (admin), POST /flashcard-reminders/send (admin), POST /flashcard-reminders/smart (admin, FSRS-based) |
...
| Learning Engine | 44 | 17 | 189 | Quizzes (4), Flashcards+FSRS (4), Concepts/Knowledge Graph (7), Streaks (2), Leaderboard (5), Discussions (5), XP (1), Badges (1), Adaptive Pre-test (3), Velocity (1), Activity Feed (2), Study Groups (6), Certificates (3) |

**Итого:** 92 endpoint, 30 таблиц, 653 тестов

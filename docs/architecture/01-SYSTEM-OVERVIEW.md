...
EduPlatform — учебная платформа с полным циклом обучения. Шесть бэкенд-сервисов, фронтенд и инфраструктура мониторинга/нагрузочного тестирования. Три роли: Student, Teacher, Admin. Студент может найти курс по категориям и фильтрам, записаться, пройти уроки, отслеживать прогресс (auto-completion при 100%), оставить отзыв. Преподаватель может создать курс с модулями и уроками. Администратор верифицирует преподавателей. Email verification и forgot password. Frontend на TanStack Query с optimistic updates и error boundaries. Оптимизирована производительность (157 RPS, p99 51ms) и надёжность (health checks, rate limiting, CORS, refresh tokens, XSS sanitization).
...
│   ▼         ▼           ▼          ▼          ▼           ▼     │
│ ┌────────┐ ┌────────┐ ┌────────┐ ┌───────┐ ┌────────┐ ┌──────┐│
│ │Identity│ │ Course │ │Enroll- │ │Payment│ │Notif.  │ │Learn.││
│ │ :8001  │ │ :8002  │ │ment    │ │ :8004 │ │ :8005  │ │:8007 ││
│ │ Python │ │ Python │ │ :8003  │ │Python │ │ Python │ │Pytho.││
│ │ FastAPI│ │ FastAPI│ │ Python │ │FastAPI│ │ FastAPI│ │FastAP││
│ └───┬────┘ └───┬────┘ └───┬────┘ └──┬────┘ └───┬────┘ └──┬───┘│
│     │          │          │         │          │          │     │
├─────┼──────────┼──────────┼─────────┼──────────┼──────────┼────┤
│     │        DATA LAYER   │         │          │          │    │
│     │          │          │         │          │          │    │
│ ┌───▼──┐ ┌───▼──┐ ┌───▼──┐ ┌───▼──┐ ┌───▼──┐ ┌───▼──┐  │
│ │iden- │ │cours-│ │enrol-│ │paym- │ │noti- │ │learn-│  │
│ │tity  │ │e db  │ │ment  │ │ent db│ │f. db │ │ing db│  │
│ │db    │ │:5434 │ │db    │ │:5436 │ │:5437 │ │:5438 │  │
│ │:5433 │ │      │ │:5435 │ │      │ │      │ │      │  │
│ └──────┘ └──────┘ └──────┘ └──────┘ └──────┘ └──────┘  │
│                                                              │
│              ┌──────────┐                                    │
...
1. **Database-per-service** — у каждого сервиса своя PostgreSQL (6 БД)
2. **Clean Architecture** — routes → services → domain ← repositories
3. **JWT shared secret** — все 6 сервисов валидируют JWT самостоятельно, без обращения к Identity
...
8. **Redis everywhere** — все 6 сервисов подключены к Redis (rate limiting + cache в Course и Learning)
9. **Health checks** — `/health/live` (liveness) + `/health/ready` (readiness) на всех 6 сервисах
...
| Health checks (/health/live, /health/ready) | — | все 6 сервисов |

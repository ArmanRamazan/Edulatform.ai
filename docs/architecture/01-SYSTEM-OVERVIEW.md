...
**Метрики:** ~130 endpoints, ~48 таблиц в 7 БД, ~740 тестов.
...
|--------|-----------|--------|--------|----------------------|
| Identity | ~27 | 8 | 92 | register, login, /me, roles, email verification, forgot password, refresh tokens, teacher verification, referrals, public profiles, follows, **organizations**, **org members** |
...
| AI Service | ~18 | — | 126 | quiz/summary generation, Socratic tutor, study-plan, content moderation, **Strategist** (path planning via LLM + RAG concepts + Learning mastery, next concept, adaptive path with remedial/skip, Redis cache 24h), **Designer** (mission generation, recaps), **Coach** (guided sessions), **daily mission**, **agent memory** |
...
**Итого (активные):** ~151 endpoints, ~48 таблиц, ~716 тестов
...
|----------|--------|---------------|
| API Gateway | Не нужен при прямом доступе к сервисам | По необходимости |

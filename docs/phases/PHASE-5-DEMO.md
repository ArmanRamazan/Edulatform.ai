# Phase 5 — Demo-Ready Product [COMPLETED]
...
- [x] **mock-mission-data** — Банк mock данных для миссий
...
- [x] **mock-search-data** — Mock данные для unified search
...
- [x] **mock-activation** — Автоматическое переключение на mock
...
- [x] **stripe-mock-mode** — Если нет `STRIPE_SECRET_KEY`:
...
- [x] **billing-frontend-guard** — Frontend billing page
...
- [x] **onboarding-layout** — `apps/buyer/app/(app)/onboarding/page.tsx`
  - Server component wrapper
  - Step indicator (1-5) с анимацией прогресса
  - Stepper bar сверху: фиолетовый градиент по заполнению

- [x] **onboarding-step1-org** — Select Organization
...
- [x] **onboarding-step2-profile** — Role & Experience
...
- [x] **onboarding-step3-pretest** — AI Pretest
...
- [x] **onboarding-step4-plan** — Plan Preview
...
- [x] **onboarding-step5-start** — Launch
...
- [x] **onboarding-redirect** — Middleware/guard
...
- [x] **reactflow-setup** — Установка и настройка
...
- [x] **concept-map-view** — Основной вид: Concept Map
...
- [x] **mind-map-view** — Zoom на концепт: Mind Map
...
- [x] **graph-page-integration** — Страница `/graph`
...
- [x] **ws-notification-backend** — WebSocket endpoint в ws-gateway
...
- [x] **ws-notification-frontend** — TopBar live badge
...
- [x] **coach-streaming-backend** — SSE endpoint в AI сервисе
  - `GET /ai/coach/stream/{session_id}` — Server-Sent Events
...
- [x] **coach-streaming-frontend** — Mission session UI
...
- [x] **seed-demo-org** — Организация "Acme Engineering"
...
- [x] **seed-rag-documents** — 5 документов в RAG
...
- [x] **seed-concepts** — Extracted concepts
...
- [x] **seed-script-update** — Обновить `tools/seed/seed.py`
...
- [x] **frontend-build-fix** — Исправить `pnpm build`
...
- [x] **route-protection** — Middleware для auth
...
- [x] **loading-polish** — Loading и transition states
...
- [x] **startup-script** — Один скрипт для запуска демо
...
- [x] `docker compose up` поднимает все 10 сервисов
- [x] `demo@acme.com` / `demo123` → login → dashboard с данными
- [x] Onboarding wizard проходится end-to-end
- [x] Knowledge Graph визуализирует 47 концептов
- [x] Mission с coach streaming работает (mock)
- [x] Flashcards review работает
- [x] Search возвращает результаты
- [x] Team Analytics показывает heatmap
- [x] Нотификации live через WebSocket
...
> 3 pre-existing failures in enrollment (known issue).

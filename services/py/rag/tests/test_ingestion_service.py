...
@pytest.fixture
def service(doc_repo, embedding_client):
...
class TestIngest:
    async def test_creates_document_and_chunks(self, service, doc_repo, embedding_client):
        doc = _make_document()
        doc_repo.create_document.return_value = doc
        doc_repo.create_chunks.return_value = [uuid4(), uuid4()]
...
class TestDelete:
    ...
    async def test_returns_false_when_not_found(self, service, doc_repo):
